...
class w2g(nn.Module):
    ...
    def __init__(self, delta_g, Gmin, G_SA00, G_SA01, G_SA10, G_SA11, weight_shape, p_SA00, p_SA01, p_SA10, p_SA11,  
                 enable_rand=True, enable_SAF=False, fault_rate = 0.5, fault_dist = "cluster", msb_only_ec = True,  device=None, config_path=None, layer_count=0):
        ...
        self.G_pos = torch.zeros(new_shape).to(self.device)
        self.G_neg = torch.zeros(new_shape).to(self.device)
        self.SAF_pos = SAF(new_shape, p_SA00=self.p_SA00, p_SA01=self.p_SA01, p_SA10=self.p_SA10, p_SA11=self.p_SA11,
                            ...
                            config_path=self.config_path, layer_count=self.layer_count)
    ...
    def forward(self, input):
        ...
        # self.G_pos = self.G_SA00 + input_scaled_pos * self.delta_g  #needs modification
        self.G_pos[input_scaled_pos == 0] = self.G_SA00
        self.G_pos[input_scaled_pos == 1] = self.G_SA01
        self.G_pos[input_scaled_pos == 2] = self.G_SA10
        self.G_pos[input_scaled_pos == 3] = self.G_SA11

        self.G_neg[input_scaled_neg == 0] = self.G_SA00
        self.G_neg[input_scaled_neg == 1] = self.G_SA01
        self.G_neg[input_scaled_neg == 2] = self.G_SA10
        self.G_neg[input_scaled_neg == 3] = self.G_SA11

        #postive_crxb = self.G_pos.view().transpose(2,3)
        #print(self.G_pos, type(self.G_pos), self.G_pos.size())
        # self.G_neg = self.G_SA00 + input_scaled_neg * self.delta_g #needs modification
        #print("diff crxb",self.G_pos)
        #quit(0)
        # the following two steps will update the SAF masking if enable_rand is True
        if self.enable_SAF:
            output = torch.cat((self.SAF_pos(self.G_pos, positive_mask).unsqueeze(0),
                                self.SAF_neg(self.G_neg, negative_mask).unsqueeze(0)),0)
