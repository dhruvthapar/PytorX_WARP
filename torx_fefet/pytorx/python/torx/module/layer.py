...
class crxb_Conv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding=0, bias=True,groups=1,dilation=1,device=None):
        ...
        self.w2g = w2g(self.delta_g, Gmin=self.Gmin, G_SA00=self.Gmin, G_SA01=self.Gmax-2*self.delta_g, G_SA10=self.Gmin+2*self.delta_g, G_SA11=self.Gmax,
                       ...
                       enable_rand=True, enable_SAF=config['enable_SAF'], fault_rate = self.fault_rate, fault_dist = self.fault_dist, msb_only_ec = self.enable_ec_SAF_msb, device=self.device)
